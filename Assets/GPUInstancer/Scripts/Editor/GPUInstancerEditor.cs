            ...
            if (showPrototypeBox)
            {
                ...
                if (!(prototype0 is GPUInstancerDetailPrototype))
                {
                    if (!isShadowCastingMixed && isShadowCasting)
                    {
                        ...
                        {
                            hasChanged |= MultiSlider(selectedPrototypeList, GPUInstancerEditorConstants.TEXT_shadowDistance, shadowDistance, 0.0f, QualitySettings.shadowDistance, shadowDistanceMixed, (p, v) => p.shadowDistance = v);
                        }
